...
List files/folders in a directory (via minibuffer):
: C-x C-d : list-directory
- temporary read-only buffer, unlike [[#dired]]
- with a /universal argument/, a verbose listing gets made (like ~ls -l~)
- with a /file name pattern/ in the minibuffer, a filter can be applied to
  listed file names (e.g. =/u2/emacs/src/*.c= to list all C files)

Create/delete directories (via minibuffer):
: M-x make-directory
: M-x delete-directory
- asks to delete recursively if directory isn’t empty
- setting the variable ~delete-by-moving-to-trash~ to ~t~ moves a removed
  directory to the trash can instead
** Comparing Files
- see [[info:emacs#Comparing Files][emacs#Comparing Files]]

Display differences between two (minibuffer-prompted) files:
: M-x diff
- opens a ~*diff*~ buffer in the major mode ~Diff Mode~
- uses GNU Diffutils

Compare contents of the current buffer with a selected file:
: M-x diff-buffer-with-file
- useful e.g. to compare unsaved changes in the buffer to its file

Compare contents of two buffers:
: M-x diff-buffers

Feature-rich alternative: [[#ediff]]
...
When a custom face has been defined (via ~defface~) and is being redefined
later (e.g. by changing and reloading the config), it may not get updated
due to caching. Clearing the face cache or resetting the face-spec fixes
that:
: (clear-face-cache)
: (face-spec-reset-face 'some-face)
...
- [[info:emacs#Fetching Package Sources][emacs#Fetching Package Sources]] (description of keywords, etc.)
...
*** [built-in] WDired (writable Dired mode)
:PROPERTIES:
:CUSTOM_ID: wdired-mode
:END:
Allows to edit filenames, link targets and file permissions from a dired
buffer directly. Applying changes modifies the files/directories on disk,
which affects any files already opened in Emacs. Deleted filenames get
flagged for deletion in Dired.

To enter wdired mode from dired mode:
: wdired-change-to-wdired-mode
- ~C-c C-c~ to exit and apply all changes
- ~C-c ESC~ to abort, discarding changes

*** [built-in] Ediff (file comparison)
:PROPERTIES:
:CUSTOM_ID: ediff
:END:
Comprehensive visual interface to Unix diff and patch utilities.
- info page: [[info:ediff#Top][ediff]]
