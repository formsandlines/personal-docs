...
tries to find a number which, when multiplied by /x mod m/, yields the
original /a mod m/.
...
| ~v x <n>~         |         | gen. vector of ~n~ indices                  |
| ~<n s i> C-u v x~ |         | gen. ~n~ items, start from ~s~, increm. by ~i~  |
| ~v b <n>~         |         | build vector from ~n~ top stack elements    |
| ~v v~             |         | reverse vector                            |
| ~v S~             |         | sort vector (increasing order)            |
| ~v r <n>~         |         | extract row/col ~n~ from vector or matrix   |
| / ~v c <n>~       |         |                                           |
|                 |         | - with /negative argument/, deletes it      |
|-----------------+---------+-------------------------------------------|
