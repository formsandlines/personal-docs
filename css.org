...
: contain :: none | size | layout | paint | strict
...
* Color Schemes
~prefers-color-scheme~ conditionally applies styles based on the users chosen
color scheme. Can be used to set the initial theme based on users system
preferences before the JS loads, preventing flash of wrong theme.
: @media (prefers-color-scheme: light) { <styles> }
: @media (prefers-color-scheme: dark) { <styles> }
- [[https://developer.mozilla.org/en-US/docs/Web/CSS/@media/prefers-color-scheme][prefers-color-scheme (mdn docs)]]

~color-scheme~ helps browsers render native UI elements (like scrollbars and
form controls) in the appropriate theme colors.
: :root { color-scheme: light dark }
- should be set dynamically to match the theme-switcher value, if present
- [[https://developer.mozilla.org/en-US/docs/Web/CSS/color-scheme][color-scheme (mdn docs)]]

~light-dark~ is a CSS color function which returns either one of its color
arguments depending on the light/dark color scheme set by the developer or the
users requested light or dark color theme. ~color-scheme~ must have a value of
~light dark~ (usually set on ~:root~).
: <prop>: light-dark(<color-light>, <color-dark>);
- no need to set theme colors in ~prefers-color-scheme~ media query
- works on all properties where any ~<color>~ value is accepted
- [[https://developer.mozilla.org/en-US/docs/Web/CSS/color_value/light-dark][light-dark (mdn docs)]]

Also see:
- [[https://css-tricks.com/almanac/properties/c/color-scheme/][color-scheme (CSS almanac / CSS-Tricks)]]

* Error handling
- [[https://developer.mozilla.org/en-US/docs/Web/CSS/CSS_syntax/Error_handling][CSS error handling (mdn docs)]]
